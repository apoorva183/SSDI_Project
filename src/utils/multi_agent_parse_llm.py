...
# Coursework removed - now handled via course search functionality
...
class ResumeModel(BaseModel):
    ...
    soft_skills: List[str] = []
    # New fields for enhanced academic history
    previous_degrees: List[str] = Field(default_factory=list)
...
IMPORTANT: Only extract information that is explicitly present in the resume. If any field is missing or unclear, leave it null. Do NOT guess, infer, or invent any details. Return ProfileInfoModel only.
"""

P_PAST_ACADEMIC = """
Extract PAST ACADEMIC HISTORY (before current program):

- previous_degrees: List of completed degrees with years if available
- previous_institutions: List of institutions attended before current program  
- graduation_years: Years of previous degree completions
- past_academic_profile_text: Summary of all prior education before current program

IMPORTANT: Only extract information that is explicitly present in the resume. If any field is missing or unclear, leave it null. Do NOT guess, infer, or invent any details. Return CourseworkModel only.
"""
...
a_courses  = Agent(Model_Name, system_prompt=P_PAST_ACADEMIC, output_type=CourseworkModel)
...
@Tool
async def extract_past_academic(resume_text: str) -> CourseworkModel:
...
controller = Agent(
    ...
    tools=[extract_profile, extract_past_academic, extract_experience,
...
def parse_resume_text_multiagent_deterministic(resume_text: str) -> ResumeModel:
    ...
    merged = ResumeModel(
        ...
        # New fields for enhanced academic history
        previous_degrees=course.previous_degrees,
        previous_institutions=course.previous_institutions,
        graduation_years=course.graduation_years,
    )
    return merged
