...
function populateForm(data) {
    ...
    // Handle new academic history fields
    if (data.previous_degrees && Array.isArray(data.previous_degrees)) {
        data.previous_degrees.forEach(degree => {
            // Add previous degrees to academic interests or certifications
            if (degree.trim()) {
                addSkill('certifications', `Previous: ${degree}`);
            }
        });
    }
    
    if (data.previous_institutions && Array.isArray(data.previous_institutions)) {
        data.previous_institutions.forEach(institution => {
            // Add previous institutions to academic interests
            if (institution.trim()) {
                addSkill('academicInterests', `Alumni: ${institution}`);
            }
        });
    }
